...
class search_paths_(Subroutine):
    def __init__(self):
        super(search_paths_, self).__init__(self.__class__.__name__)
    ...
    def find_dir(self, sl_name, search_seg_ptr, entryname, dir_name, code):
        ...
        if code.val == 0:
            for path in sl_info_get.ptr.paths:
                ...
                native_path = GlobalEnvironment.fs.path2path(path, entryname)
    ...
    def set(self, sl_name, search_seg_ptr, sl_info_ptr, code):
        ...
        if sl_info_ptr != null():
            for path in sl_info_ptr.data.paths:
                if path.pathname in SEARCH_PATH_SYMBOLS or GlobalEnvironment.fs.file_exists(path.pathname):
                    path.code = 0
                else:
